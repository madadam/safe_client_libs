...
use self::account::{Account, ClientKeys, Dir};
#[cfg(feature = "use-mock-routing")]
...
impl Client {
        ...
        where T: 'static
    {
        ...
        let config_dir = Dir::random(0);

        let acc = Account::new(user_root, config_dir);
        let pub_key = acc.maid_keys.sign_pk.clone();
        ...
        let (mut routing, routing_rx) = setup_routing(Some(acc.maid_keys.clone().into()))?;
